#include <stdio.h>
#include <stdlib.h>
#include "list.h"

int main() {
  int x;
  printf("Write your Tests for your linked list implementation\n");
  list_t *mylist;
  mylist = list_alloc();
  list_add_to_front(mylist, 10);
  list_add_to_front(mylist, 20);
  list_add_to_front(mylist, 30);
  list_add_to_front(mylist, 40);
  list_add_to_front(mylist, 50);
  list_add_to_front(mylist, 60);
	printf("Current list: ");
	list_print(mylist);
	printf("\n");
	printf("This is the list with index 5 removed: ");
	list_remove_at_index(mylist, 5);
	list_print(mylist);
	printf("\n");
	printf("The length of this list is: %d\n", list_length(mylist));

  list_add_to_front(mylist, 80);
  list_add_to_front(mylist, 90);

  list_add_to_front(mylist, 100);
	printf("After adding to front of list: ");
	list_print(mylist);
	printf("\n");

	printf("After adding to back of list: ");
  list_add_to_back(mylist, 39);  
	list_add_to_back(mylist, 42);
  list_add_to_back(mylist, 18);  
  list_add_to_back(mylist, 190);
	list_print(mylist);
	printf("\n");	
	list_add_at_index(mylist, 93, 1);
	printf("The list after %d is added at index %d: ", 93, 1);
	list_print(mylist);;
	printf("\n");

	printf("The length of this list is: %d\n", list_length(mylist));
	printf("Is %d in the list?: %d\n", 100, list_is_in(mylist, 100));
	printf("Is %d in the list?: %d\n", 38, list_is_in(mylist, 38));

	list_free(mylist);
	printf("Free the list: ");
  list_print(mylist);
	printf("\n");

	list_add_to_front(mylist, 81);
  list_add_to_back(mylist, 4);
  list_add_to_front(mylist, 308);
  list_add_to_back(mylist, 70);
  list_add_to_front(mylist, 290);
	printf("Current list: ");
  list_print(mylist);
	printf("\n");
	printf("The value at index %d is: %d\n", 1, list_get_elem_at(mylist, 1));
	printf("Value %d is at index: %d\n", 70, list_get_index_of(mylist, 70));

	list_remove_at_index(mylist, 3);
	printf("This is the list with index 3 removed: ");
	list_print(mylist);
	printf("\n");

	printf("Add to the front of the list: ");
	list_add_to_front(mylist, 10);
	list_add_to_front(mylist, 20);
	list_print(mylist);
	printf("\n");

	printf("This is the list before %d is added: ", 77);
	list_print(mylist);
	printf("\n");
	list_add_at_index(mylist, 77, 3);
	printf("This is the list after %d is added at index %d: ", 77, 3);
	list_print(mylist);;
	printf("\n");

	list_remove_from_back(mylist);
	printf("Remove from back of the list: ");
	list_print(mylist);
	printf("\n");

	printf("Remove from front of the list: ");
	list_remove_from_front(mylist);
	list_print(mylist);
	printf("\n");

  printf("Is %d in the list?: %d\n", 10, list_is_in(mylist, 10));
	printf("Is %d in the list?: %d\n", 15, list_is_in(mylist, 15));

  list_add_to_front(mylist, 54);
  list_add_to_front(mylist, 57);
  list_add_to_front(mylist, 29);
  list_add_to_front(mylist, 35);
  list_add_to_front(mylist, 20);
  list_add_to_front(mylist, 73);
	printf("Current list: ");
	list_print(mylist);
	printf("\n");
	printf("The value at index %d is: %d\n", 4, list_get_elem_at(mylist, 4));
	printf("Value %d is at index: %d\n", 54, list_get_index_of(mylist, 54));


  list_remove_at_index(mylist, 2);
  printf("This is the list with index 2 removed: ");
	list_print(mylist);
	printf("\n");

  list_remove_at_index(mylist, 4);
  printf("This is the list with index 4 removed: ");
	list_print(mylist);
	printf("\n");
  printf("The list length is: %d\n", list_length(mylist));

	list_add_to_front(mylist, 4);
  list_add_at_index(mylist, 21, 1);
  list_add_at_index(mylist, 65, 1);
  list_add_at_index(mylist, 10, 2);
	list_add_at_index(mylist, 93, 4);
  list_add_at_index(mylist, 43, 3);
	printf("Current list: ");
  list_print(mylist);
	printf("\n");

	list_remove_from_back(mylist);
	printf("Remove from back of the list: ");
	list_print(mylist);
	printf("\n");
  list_remove_from_front(mylist);
	printf("Remove from front of the list: ");
	list_print(mylist);
	printf("\n");
  list_remove_at_index(mylist, 2);
	printf("This is the list with index 2 removed: ");
	list_print(mylist);
	printf("\n");

	list_free(mylist);
	printf("Free the list: ");
  list_print(mylist);
	printf("\n");

	mylist = list_alloc();
	list_add_to_front(mylist, 21);
  list_add_to_front(mylist, 5);
  list_add_to_front(mylist, -5);
  list_add_to_front(mylist, 35);
  list_add_to_front(mylist, 20);
  list_add_to_front(mylist, 70);
	printf("Current list: ");
  list_print(mylist);
	printf("\n");

  printf("Is %d in the list?: %d\n", 21, list_is_in(mylist, 21));
  printf("Value at index %d in the list?: %d\n", 3, list_get_elem_at(mylist, 3));
  printf("Value at index %d in the list?: %d\n", 5, list_get_elem_at(mylist, 5));
  printf("Value at index %d in the list?: %d\n", 2, list_get_elem_at(mylist, 2));
  printf("Index of %d?: %d\n", 70, list_get_index_of(mylist, 70));
  printf("Index of %d?: %d\n", 5, list_get_index_of(mylist, 5));
  printf("Index of %d?: %d\n", 20, list_get_index_of(mylist, 20));
  
  list_add_at_index(mylist, -1, 1);
  printf("This is the list after %d is added at index %d: ", -1, 1);
	list_print(mylist);;
	printf("\n");
  list_add_at_index(mylist, 28, 4);
  printf("This is the list after %d is added at index %d: ", 28, 4);
	list_print(mylist);;
	printf("\n");
  list_add_at_index(mylist, 10, 2);
  printf("This is the list after %d is added at index %d: ", 10, 2);
	list_print(mylist);;
	printf("\n");
  printf("The list length is: %d\n", list_length(mylist));

	list_remove_from_back(mylist);
	printf("Remove from back of the list: ");
	list_print(mylist);
	printf("\n");
  list_remove_from_front(mylist);
	printf("Remove from front of the list: ");
	list_print(mylist);
	printf("\n");
  list_remove_at_index(mylist, 3);
	printf("Remove from index 3 of the list: ");
	list_print(mylist);
	printf("\n");
  list_remove_at_index(mylist, 2);
	printf("Remove from index 2 of the list: ");
	list_print(mylist);
	printf("\n");

}
// list_add_to_front(mylist, 10);
//   list_add_to_front(mylist, 20);
//   list_add_to_front(mylist, 30);
//   list_add_to_front(mylist, 40);
//   list_add_to_front(mylist, 60);
// 	printf("Current list: ");
// 	list_print(mylist);
// 	printf("\n");
//   list_print(mylist);
//   // list_remove_from_front(mylist);
//   list_print(mylist);
//   list_add_to_back(mylist, 13);  
//   list_print(mylist);
//   // list_remove_from_back(mylist);
//   list_print(mylist);

//   list_add_to_front(mylist, 10);
//   list_add_to_front(mylist, 20);
//   list_add_to_front(mylist, 30);
//   list_add_to_front(mylist, 40);
//   list_add_to_front(mylist, 60);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 0);
//   list_print(mylist);
//   // list_remove_at_index(mylist, -2);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 7);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 4);
//   list_print(mylist);

//   list_free(mylist);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 0);
//   list_print(mylist);
//   list_add_to_front(mylist, 60);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 1);
//   list_print(mylist);
//   list_add_to_front(mylist, 80);
//   list_print(mylist);
//   // list_remove_at_index(mylist, 0);
//   list_print(mylist);

//   // printf("Is %d in the list?: %d\n", 60, list_is_in(mylist, 60));
//   list_add_to_back(mylist, 50);
//   list_add_to_back(mylist, 60);
//   list_add_to_back(mylist, 70);
//   list_add_to_back(mylist, 80);
//   list_add_to_back(mylist, 90);
//   list_print(mylist);
//   // printf("Is %d in the list?: %d\n", 30, list_is_in(mylist, 30));
//   // printf("Is %d in the list?: %d\n", 60, list_is_in(mylist, 60));
//   // printf("Is %d in the list?: %d\n", 80, list_is_in(mylist, 80));
//   // list_add_to_back(mylist, 70);
//   list_print(mylist);
//   // printf("Is %d in the list?: %d\n", 70, list_is_in(mylist, 70));
  
//   // printf("The list length is %d\n", list_length(mylist));
//   // printf("Value at %d in the list?: %d\n", -4, list_get_elem_at(mylist, -4));
//   // printf("Value at %d in the list?: %d\n", 10, list_get_elem_at(mylist, 10));
//   // printf("Value at %d in the list?: %d\n", 6, list_get_elem_at(mylist, 6));
//   // printf("Value at %d in the list?: %d\n", 5, list_get_elem_at(mylist, 5));
//   // printf("Value at %d in the list?: %d\n", 0, list_get_elem_at(mylist, 0));
//   list_free(mylist);
//   list_print(mylist);
//   // printf("Value at %d in the list?: %d\n", 0, list_get_elem_at(mylist, 0));
//   // list_remove_at_index(mylist, 0);
//   // printf("Is %d in the list?: %d\n", 21, list_is_in(mylist, 21));
//   // printf("Index of %d?: %d\n", 21, list_get_index_of(mylist, 21));
//   list_add_to_front(mylist, 10);
//   list_add_to_front(mylist, 20);
//   list_add_to_front(mylist, 30);
//   list_add_to_front(mylist, 40);
//   list_add_to_front(mylist, 50);
//   list_print(mylist);
//   // printf("Index of %d?: %d\n", 50, list_get_index_of(mylist, 50));
//   // printf("Index of %d?: %d\n", 81, list_get_index_of(mylist, 81));
//   // printf("Index of %d?: %d\n", 10, list_get_index_of(mylist, 10));
//   // printf("Index of %d?: %d\n", 30, list_get_index_of(mylist, 30));
//   list_add_to_front(mylist, 60);
//   list_print(mylist);
//   // printf("Index of %d?: %d\n", 50, list_get_index_of(mylist, 50));
//   // printf("Index of %d?: %d\n", 60, list_get_index_of(mylist, 60));
//   list_add_to_front(mylist, 10);
//   list_print(mylist);
//   // printf("Index of %d?: %d\n", 10, list_get_index_of(mylist, 10));
//   list_add_to_back(mylist, 40);
//   // list_print(mylist);
//   // printf("Index of %d?: %d\n", 40, list_get_index_of(mylist, 40));
//   return 0;
// }
